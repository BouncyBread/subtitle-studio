...
# Adapted from mlx-whisper 0.4.3 (MIT; see LICENSE-MLX).
# Local changes: absolute imports and an optional finalized-window callback.
# The decoding algorithm and model are unchanged.
    ...
    **decode_options,
    ...
    def publish_window():
        if segment_callback is not None:
            segment_callback(all_segments, min(content_frames, seek) / FRAMES_PER_SECOND,
                             content_frames / FRAMES_PER_SECOND, language)
            ...
            while seek < seek_clip_end:
                ...
                if no_speech_threshold is not None:
                    ...
                    if should_skip:
                        ...
                        publish_window()
                        pbar.update(min(content_frames, seek) - skipped_from)
                        continue
                ...
                # Publish only after the decoder has finalized this window.
                publish_window()
